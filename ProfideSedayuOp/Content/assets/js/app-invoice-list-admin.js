...
$(function () {
    ...
    let datadone = [];
    $.ajax({
        url: '/Admin/GetDataPengecekan',   //'/Home/GetDataPengecekan', // Ganti dengan endpoint server Anda
        method: 'POST',  //'POST
        data: {
            User_Pengaju: '',
            User_Group: '',
            ...
            startDate: '',
            endDate: '',
            Asal: 'PT Setia Darma Barokah',
        },
        //"PT SEDAYU DANA BANDA"
        success: function (responseDot) {
            if (responseDot && Array.isArray(responseDot)) {
                $.ajax({
                    url: '/Admin/GetDataPengecekan',   //'/Home/GetDataPengecekan', // Ganti dengan endpoint server Anda
                    method: 'POST',  //'POST',
                    data: {
                        ...
                        startDate: '',
                        endDate: '',
                        Asal: 'PT SEDAYU DANA BANDA',
                    },
                    success: function (responseDot2) {
                        const combinedata = [...responseDot, ...responseDot2];
                        datadone = combinedata
                        console.log('Combined Data:', combinedata); // Debugging
                        tampiltable(datadone)
                    }
                });
            }
        }
    ...
    // Invoice datatable
    function tampiltable(data) {
        var dt_invoice = dt_invoice_table.DataTable({
            //ajax: //assetsPath + 'json/invoice-list.json', // JSON file to add data
            //{
            //    url: '/Admin/GetDataPengecekan', // URL endpoint yang digunakan
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        var sumAsal = full['Client'];
                        let $Asal = '' 
                        
                        if (sumAsal === "PT. Mitsui Leasing Capital Indonesia") {
                            $Asal = 'PT SEDAYU DANA BANDA';
                        } else {
                            $Asal = 'PT Setia Darma Barokah';
                        }
                        return (
                            ...
                            `<button data-bs-toggle="tooltip" class="btn btn-sm btn-icon btn-te+xt-secondary rounded-pill waves-effect waves-light delete-record" data-bs-placement="top" data-file="${$datadown}" data-asal="${$Asal}" title="Download Invoice"><i class="ri-folder-download-line ri-22px"></i></button>`
        ...
        $('.invoice-list-table tbody').on('click', '.delete-record', function () {
            ...
            $.ajax({
                url: '/Admin/GetDataPengecekan', // Ganti dengan endpoint server Anda
                method: 'POST',
                dataType: 'json',
                data: {
                    UserID: '',
