...
$(function () {
    ...
    let datadone = [];
    $.ajax({
        url: assetsPath + 'json/BackDataDone-Mit.json',   //'/Home/GetDataPengecekan', // Ganti dengan endpoint server Anda
        method: "GET",  //'POST',
        dataType: 'json',
        //data: {
        //    User_Pengaju: '',
            ...
            columnDefs: [
...

                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        return `<button  class="download-link btn btn-success" data-asal="${$asal}" data-file="${$file}" data-no="${$no_trns}" data-nama="${$nama}" data-user="${$user}" data-wilayah="${$wilayah}" data-cabang = "${$cabang}" data-tanggal= "${moment($TanggalPengajuan).format('YYYY-MM-DD')}"><i class="ri-download-cloud-2-line"></i></button>`;
                    },
                    searchable: false
                },
                {
